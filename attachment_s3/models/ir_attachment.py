...
class IrAttachment(models.Model):
    ...
    def _get_stores(self):
        store_location = ["s3"]
        store_location += super()._get_stores()
    ...
    def _get_s3_bucket(self, name=None):
        ...
        * ``AWS_BUCKETNAME``
        ...
        if not (access_key and secret_key and bucket_name):
            msg = _(
                "If you want to read from the %(bucket)s S3 bucket, the following "
                "environment variables must be set:\n* AWS_ACCESS_KEY_ID\n* "
                "AWS_SECRET_ACCESS_KEY\nIf you want to write in the %(bucket)s S3 bucket, "
                "this variable must be set as well:\n* AWS_BUCKETNAME\nOptionally, the "
                "S3 host can be changed with:\n* AWS_HOST\n"
            ) % {"bucket": bucket_name}
